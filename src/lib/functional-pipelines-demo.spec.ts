...
import type { GenericGraph, GraphEvent, NodeId, XChainId, YChainId } from '@functional-pipelines-demo/source';
import {
  reduceToXYMutableNaive0,
  reduceToXYMutableNaive1,
  reduceToXYMutableNaive2, reduceToXYMutableNaive3
...
const getWidth = flow(RA.map(flow(STR.split(''), RA.size)), ns => Math.max(...ns) as Dimension);

const toTokenMatrix = flow((s: string) => pipe(
  s, STR.split('\n'), ss => pipe(ss, RA.map(flow(((s) => (w: number) => s.padEnd(w, SPACE)), apply(getWidth(ss)), STR.split(''), RA.map(flow(s => s as Token, S.decodeSync(Token)))))),
));
...
const tokenMatrixToIndices = (ts: readonly (readonly Token[])[]) => {
  ...
  for (let yi_ = 0; yi_ < ts.length; yi_++) {
    ...
    for (let xi_ = 0; xi_ < row.length; xi_++) {
      ...
      const token = row[xi_]!;
      if (isXSpan && (token !== X_LINE && token !== SPACE)) throw new Error(`Expected ${X_LINE} but got ${token}`);
      if (isYSpan && (token !== Y_LINE && token !== SPACE)) throw new Error(`Expected ${Y_LINE} but got ${token}`);
...
type: S.literal('xAdded'),
    ...
    chainId: XChainId
 */
const xIndicesToEvents = (xChains: number[][]): readonly GraphEvent[] => pipe(xChains,
  RA.mapWithIndex((yi, xChain) => pipe(xChain,
...
const yIndicesToEvents = (yChains: number[][]): readonly GraphEvent[] => pipe(yChains,
  RA.mapWithIndex((xi, yChain) => pipe(yChain,
...
const indicesToEvents = ({ xChains, yChains }: ReturnType<typeof tokenMatrixToIndices>): readonly GraphEvent[] => pipe([
...
const trimNewlines = flow(STR.split('\n'), a => pipe(a, RA.filterWithIndex((i, x) => (i !== 0 && i !== a.length - 1) || x.trim().length > 0)),
  a => a.join('\n'));

const asciiToEvents = flow(trimNewlines, toTokenMatrix, tokenMatrixToIndices, indicesToEvents)
...
const toAscii = (gg: GenericGraph): string => {
  ...
  const matrix: Token[][] = new Array(spannedHeight * 2).fill(null).map(() => new Array(spannedWidth * 2).fill(SPACE));
  const xsOrdered: [XChainId, NodeId[]][] = [...gg.x.entries()].sort(([a], [b]) => STR.Ord.compare(a, b));
  ...
  for (let i__ = 0; i__ < spannedHeight; i__++) {
    for (let j__ = 0; j__ < spannedWidth; j__++) {
      ...
      const yChain = [...gg.y.entries()].find(([yChainId, a]) => a.find(({xChainId, nodeId: nodeId_})=>xChainId === xChainId_ && nodeId_ === nodeId));
      ...
      if (hasNode) {
        ...
        const drawExistingNodes = () => {
          ...
          if (yChain) {
            const isLastY = yChain[1].findIndex(({xChainId, nodeId: nodeId_}) => xChainId === xChainId_ && nodeId_ === nodeId) === yChain[1].length - 1;
            if (!isLastY) {
              matrix[bottomSpannedCell[0]]![bottomSpannedCell[1]] = Y_LINE;
            }
          }

          const isLastX = xLine.findIndex(x => x === nodeId) === xLine.length - 1;
          if (!isLastX) {
        ...
        const drawEmptyXDashes = () => {
          ...
          if (hasNextX && hasPrevX) { // no want to connect nothing with something
        ...
        const drawEmptyYDashes = () => {
          ...
          for (let nextI__ = i__ + 1; nextI__ < spannedHeight; nextI__++) {
            nextNodeId = xsOrdered[nextI__]![1].find(x => x === nodesOrdered[j__]);
            if (nextNodeId) break;
          }
          let prevNodeId = undefined;
          // TODO a bit wasteful
          for (let prevI__ = i__ - 1; prevI__ >= 0; prevI__--) {
            prevNodeId = xsOrdered[prevI__]![1].find(x => x === nodesOrdered[j__]);
            if (prevNodeId) break;
          }
          const hasNextY = nextNodeId !== undefined;
          const hasPrevY = prevNodeId !== undefined;
          if (hasNextY && hasPrevY) { // no want to connect nothing with something
            // draw dash right here
  ...
  return pipe(matrix, A.map(row => row.join('')), a=>a.join('\n'), normalizeAscii);
}
...
describe('functionalPipelinesDemo', () => {
  const ASCII = `
o-x-o-x-o-x
  |       |
o-x-o     |
...
`;
  ...
  it('naive valid events should work', () => {
    expect(pipe(ASCII_EVENTS, reduceToXYMutableNaive0, toAscii)).toEqual(normalizeAscii(ASCII));
  });

  describe("nodes in one x chain have to be unique", () => {
    it('naive dupe node events shouldnt work', () => {
      const xToTackleO = pipe(ASCII_EVENTS, RA.findFirst((e): e is typeof e & {type: 'xAdded'} => e.type === 'xAdded'))
      if (isNone(xToTackleO)) throw new Error('Expected xToTackleO to be defined');
      const xToTackle = xToTackleO.value;
      const xChainIdToTackle = xToTackle.chainId;
      const nodeIdToTackle = xToTackle.id;
      const graph = pipe(ASCII_EVENTS, RA.appendW({
        type: 'xAdded',
        id: nodeIdToTackle,
        chainId: xChainIdToTackle
      } satisfies GraphEvent), reduceToXYMutableNaive0);
      /*artifacts appear*/
      expect(pipe(`
o-x-o-x-o-x-   
  |       |    
o-x-o     |    
  |       |    
  x-------x-o-o
`, trimNewlines)).toEqual(pipe(graph, toAscii, normalizeAscii));
      expect(graph.x.get(xChainIdToTackle)!.length).toEqual(7);/*one too many*/
    });
    it('naive dupe node events shouldnt work with naive1', () => {
      const xToTackleO = pipe(ASCII_EVENTS, RA.findFirst((e): e is typeof e & {type: 'xAdded'} => e.type === 'xAdded'))
      if (isNone(xToTackleO)) throw new Error('Expected xIdToTackleO to be defined');
      const xToTackle = xToTackleO.value;
      const xChainIdToTackle = xToTackle.chainId;
      const nodeIdToTackle = xToTackle.id;
      expect(() => pipe(ASCII_EVENTS, RA.appendW({
        type: 'xAdded',
        id: nodeIdToTackle,
        chainId: xChainIdToTackle
      } satisfies GraphEvent), reduceToXYMutableNaive1)).toThrowError('duplicate');
    });
  });
  describe('on one y chain, nodeId is always fixed', () => {
    it('naive dupe y node events shouldnt work', () => {
      const yToTackleO = pipe(ASCII_EVENTS, RA.findFirst((e): e is typeof e & {type: 'yAdded'} => e.type === 'yAdded'))
      if (isNone(yToTackleO)) throw new Error('Expected yToTackleO to be defined');
      ...
      } satisfies GraphEvent), reduceToXYMutableNaive1);
      /*artifacts appear*/
      expect(pipe(`
o-x-o-x-o-x    
  |       |    
o-x-o     |    
  ...
  |            
`, trimNewlines)).toEqual(pipe(graph, toAscii, normalizeAscii));
      expect(graph.y.get(yToTackle.yChainId)!.length).toEqual(4);/*one too many*/
    });
    it('naive dupe y node events shouldnt work with naive2', () => {
      ...
      expect(() => pipe(ASCII_EVENTS, RA.appendW({
        ...
        xChainId: yToTackle.xChainId,
        yChainId: yToTackle.yChainId
      } satisfies GraphEvent), reduceToXYMutableNaive2)).toThrowError('duplicate');
    })
  });
...
`
      ...
      } satisfies GraphEvent), reduceToXYMutableNaive2);
      /*artifacts appear*/
      expect(pipe(`
o-x-o-x-o-x    
  |   |   |    
o-x-o     |    
  ...
  x-------x-o-o
`, trimNewlines)).toEqual(pipe(graph, toAscii, normalizeAscii));
      expect(graph.y.get(yToTackleChainId)!.length).toEqual(2); /* should be impossible cause no X to attach to */
    });
    it('naive attach y node on non-existing in X nodeId events shouldnt work with naive3', () => {
      ...
      expect(() => pipe(ASCII_EVENTS, RA.appendW({
        ...
        xChainId: xToTackleChainId,
        yChainId: yToTackleChainId
      } satisfies GraphEvent), reduceToXYMutableNaive3)).toThrowError('not initialized');
    });
  });
