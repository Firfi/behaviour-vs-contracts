...
export const reduceToXYMutableNaive0 = (events: readonly GraphEvent[]): GenericGraph => {
  const xs: Map<XChainId, NodeId[]> = new Map();
  const ys: Map<YChainId, {
    xChainId: XChainId,
    nodeId: NodeId
  }[]> = new Map();
...
const orderedSetOnce = <T>() => {
  ...
  const api = (os: OrderedSet<T>) => ({
    add: (t: T) => {
      // TODO Either
      if (os.has(t)) throw new Error(`panic! duplicate ${t} in set; allowed only once`);
      return api(os.add(t));
    },
    toArray: () => os.toArray(),
  });
...
export const reduceToXYMutableNaive1 = (events: readonly GraphEvent[]): GenericGraph => {
...
const mapOnce = <K, V>() => {
    ...
    const api = (m: Map<K, V>) => ({
      set: (k: K, v: V) => {
        // TODO Either
        if (m.has(k)) throw new Error(`panic! duplicate ${k} in map; allowed only once`);
...
export const reduceToXYMutableNaive2 = (events: readonly GraphEvent[]): GenericGraph => {
  const xs: Map<XChainId, OrderedSetOnce<NodeId>> = new Map();
  ...
  return {
    x: pipe(xs, MAP.map(s => s.toArray())),
    y: pipe(ys, MAP.map(m => [...m.entries()].map(e => ({
      xChainId: e[0],
      nodeId: e[1]
    }))))
  }
...
export const reduceToXYMutableNaive3 = (events: readonly GraphEvent[]): GenericGraph => {
  ...
  return {
    x: pipe(chains, MAP.map(c => c.keys())),
    y: pipe([...chains.entries()], A.map(e => {
      const xChainId = e[0];
      return pipe([...e[1].entries()], A.filterMap(e => pipe(e[1], O.map(yChainId => ({
        xChainId,
        nodeId: e[0],
        yChainId
      })))), a => a);
    }), A.flatten, NEA.groupBy(e => e.yChainId), REC.map(NEA.map(e => ({
      xChainId: e.xChainId,
      nodeId: e.nodeId
    }))), REC.toEntries, es => es as [YChainId, typeof es[number][1]][], es => new Map(es))
  }
}
